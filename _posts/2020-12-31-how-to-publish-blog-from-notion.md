...
I want to publish blog from my Notion content and I found a way after researching available notion hosting solutions. In this post, I will why I decided to use notablog for my blog and how you can also publish your blog from Notion for free.
...
It is created from this [shared notion page](https://www.notion.so/erajasekar/91b64f91c8904a36b63b3691cea91aa6?v=3c062b5462a64f6c85720b544a2b6ecd). Each [page in notion table](https://www.notion.so/Thirukkural-How-to-control-Anger-e26960d655a44dcc81775f824d7997aa) is published as a [blog post](https://katranaithoorumarivu.com/thirukkural-adhigaram-vegulamai.html). Read on to understand how this works.
...
* Since Notion doesn't have good backup solution, I don't want blog to use same content in Notion, instead import the content to hosting site. So that I won't the content of the blog will remain even if I lose Notions's content.
...
Then I got an idea that I should look for static site generators that can create blog from Notion content. First, I found [notion-blog](https://github.com/ijjk/notion-blog). I liked the idea, but it didn't support all the content types. For e.g toggle list didn't work. Also, it's not easy to customize theme.
...
Before adding metatags, let's look at the Notablog's directory structure and understand theme system.
...
Notablog ships with one theme `pure` which is used by default. The theme name is configured in `notablog-starter/config.json`. 
...
Figured that I need to update these template files to customize generated html content. To easily track the changes I make, I thought it's good idea to use my own theme. So
...
Then, I added following snippet to `index.html` , `post.html` files in `themes/kartranai` dir to add meta property for `og:image`.
...
Hope this post will help you build your own blog from Notion content. You can find full source of the code in [Github](https://github.com/erajasekar/notablog-starter).
